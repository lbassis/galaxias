...
\subsection{Herança}
    ...
    Além disso, foi implementada a classe abstrata \textit{PointAbs}, da qual a classe \textit{Point}
    herda. Com essa classe, a implementação já fica pronta para uma simulação com pontos em
    três dimensões, uma possível extensão da aplicação atual.
...
\begin{figure}[htb]
    ...
    \fbox{
        \includegraphics[width=20cm,height=10cm,keepaspectratio]{images/barnesPlot.eps}
    }
    \label{fig:grafico}
\end{figure}
...
Nessa versão, a função \textit{draw}, presente
...
\textit{Stack.r}, \textit{PointAbs.R}, \textit{Point.r}, \textit{Quad.r}, \textit{Particle.R}, \textit{Qnode.R} e
...
Em ambas as versões, caso o arquivo de projeto não seja aberto no \textit{Rstudio},
 o resultado é armazenado no arquivo \textit{home/result.gif}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Conclusões
%
\chapter{CONCLUSÃO}
